---
description: Set cc-reload's context budget (% of window) and its escalation mode (notify | checkpoint) for this project
argument-hint: <percent 1-95 | "off" | "notify" | "checkpoint"> e.g. 30
allowed-tools: Read, Write, Edit, Bash
---

# Set the reload budget

When context crosses the budget, cc-reload either **notifies** you to run `/checkpoint` + `/clear`
(default — one status line, never interrupts, zero model tokens) or **checkpoints automatically**
(`checkpoint` mode: forces a digest-writing turn, then asks you to `/clear`). Lower the budget for
...
2. If `$ARGUMENTS` is `notify` or `checkpoint`, set the mode instead of the percentage:
