...
- 2026-09-02 P2: README config example REPRODUCED unparseable: the three-line block at README.md:178-180 copied verbatim into `.reload/config` reads back as `45       # act at this %…`, `notify  # notify…`, `1000000      # AUTHORITATIVE…` through lib.sh `cfg`, `reload-config.sh get` AND statusline.sh. Consequence measured: `context_window: 200000  # …` pinned, stamp 1M, 150k used (75% of the pin) → hook silent (pin dropped, 15% of 1M).
...
- **Severity:** P2
- **Confidence:** high
- **Claim tag:** CONFIRMED — mechanism reproduced in the scratchpad (P2 log entry "IC1 REPRODUCED"); the FIELD trigger is version-dependent: this session's Claude Code writes subagent rows to `<sid>/subagents/*.jsonl`, while cc-repete's measured corpus (`cc-repete-plugin/hooks/stop-hook.sh:384`, "500 trailing sidechain lines") shows in-file `isSidechain:true` rows on the maintainer's versions
...
- **Location:** hooks/lib.sh:44-48 (`kv()`), scripts/reload-config.sh:36-37 (`get`), scripts/statusline.sh:48-49, :55-56, :76-77 (three inline copies); README.md:178-180 (the example block)
- **Severity:** P2
- **Confidence:** high
